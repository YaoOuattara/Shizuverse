...
if not DATABASE_URL:
DATABASE_URL = os.getenv("REMOTE_DATABASE_URL") if ENV == "production" else os.getenv("LO
if not DATABASE_URL:
...
from shizuverse.routes.socket_chat import create_socket_instance, socket_chat_bp # noqa: E40
...
socketio = SocketIO(app, async_mode="gevent", cors_allowed_origins="*", logger=True, engi
create_socket_instance(socketio)
# ── Blueprint registration ─────────────────────────────────────────────────
# auth and socket_chat registered explicitly above; chat is served via
# api_blueprint at /api/chat — skip all three in the loop to avoid collisions.
...
featured_services = []
testimonials = [
{"name": "Sarah M.", "image": url_for("static", filename="img/testimonials/user1.
"comment": "Excellent service! The booking process was seamless."},
{"name": "John D.", "image": url_for("static", filename="img/testimonials/user2.j
"comment": "Very reliable platform. Found a great handyman."},
{"name": "Emma R.", "image": url_for("static", filename="img/testimonials/user3.j
"comment": "Great experience overall. Will definitely use again!"},
]
return render_template("index.html", featured_services=featured_services, testimonial
@app.route("/health")
def health():
try:
