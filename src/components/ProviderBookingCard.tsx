        ...
        <CardContent className="p-4 sm:p-5">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              {/* Dossier remboursé : le reçu raconte la MÊME histoire que le
                  WhatsApp déjà reçu (shizu_dispute_no_payment_provider_fr,
                  « clôturée sans règlement »). Montant ABSENT, pas barré : le
                  barré est le langage visuel de la promo, et laisser le chiffre
                  sous les yeux invite à contester un montant qui n'est pas dû —
                  le message n'en cite pas non plus. */}
              {booking.payment_status === "refunded" ? (
                <span className="inline-flex items-center gap-1.5 mt-1 text-xs font-medium bg-gray-100 text-gray-700 dark:bg-gray-900/30 dark:text-gray-400 px-2.5 py-1 rounded-full"
                      data-testid={`badge-refunded-${booking.id}`}>
                  <XCircle className="h-3.5 w-3.5" />
                  Dossier remboursé — sans règlement
                </span>
