        ...
        {searched && !loading && (
          clients.length === 0 ? (
            ...
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground">{clients.length} client{clients.length > 1 ? "s" : ""} trouvé{clients.length > 1 ? "s" : ""}</p>
              {clients.map(c => (
                <div key={c.id} className="rounded-xl border border-border bg-card p-4 flex items-start justify-between gap-4">
                  <div className="space-y-1.5 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      ...
                      <span>{c.booking_count} réservation{c.booking_count !== 1 ? "s" : ""}</span>
                    </div>
                  </div>
                  <button
                    onClick={() => openModal(c)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4"
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Client : <span className="font-semibold text-foreground">{selected.name || selected.phone}</span>
            </p>
